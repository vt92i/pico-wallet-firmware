...
bool handle_initialize_wallet(const apdu_packet_t* packet, apdu_buffer_t* response) {
  ...
  if (xQueueReceive(smartcard_tx_queue, &resp, portMAX_DELAY) != pdPASS) {
    apdu_build_response(response, APDU_SW_EXECUTION_ERROR, NULL, 0);
    return false;
  }

  apdu_build_response(response, resp.status == SMARTCARD_STATUS_ERROR ? APDU_SW_EXECUTION_ERROR : APDU_SW_WAITING,
...
bool handle_reset_wallet(const apdu_packet_t* packet, apdu_buffer_t* response) {
  ...
  apdu_build_response(response, resp.status == SMARTCARD_STATUS_ERROR ? APDU_SW_EXECUTION_ERROR : APDU_SW_OK,
                      &resp.data, resp.data_len);
  return true;
}
